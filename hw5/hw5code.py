...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    p_l = prefix_sum[:-1] / Rl
    p_r = np.cumsum(sorted_target[:0:-1])[::-1] / Rr


    Hl = 1 - p_l**2 - (1 - p_l)**2
    Hr = 1 - p_r**2 - (1 - p_r)**2

    ginis =  -1 * Rl / R * Hl - Rr / R * Hr
    
    ginis = ginis[np.cumsum(counts)[:-1] - 1 if len(np.cumsum(counts)[:-1]) != 0 else [0]]
...
class DecisionTree:
    ...
    def _fit_node(self, sub_X, sub_y, node, cur_depth):
        ...
        if ((self._max_depth and cur_depth >= self._max_depth) or(self._min_samples_split and len(sub_X) < self._min_samples_split) \
