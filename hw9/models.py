...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        for sentence in parallel_corpus:
            ...
            posteriors.append(theta / np.sum(theta, axis=0))
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for i in range(len(posteriors)):
            src_idx = parallel_corpus[i].source_tokens
            tgt_idx =  parallel_corpus[i].target_tokens
            res += np.sum(posteriors[i] * (np.log(self.translation_probs[np.ix_(src_idx, tgt_idx)] + 1e-8) - np.log(posteriors[i] + 1e-8))) - (len(tgt_idx) * np.log(len(src_idx) + 1e-8))
        return res
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for i in range(len(parallel_corpus)):
            src_idx = parallel_corpus[i].source_tokens
            tgt_idx = parallel_corpus[i].target_tokens
            np.add.at(self.translation_probs, np.ix_(src_idx, tgt_idx), posteriors[i])
    ...
    def align(self, sentences):
        ...
        for sentence in sentences:
            alignment = []
            for i, tgt in enumerate(sentence.target_tokens, start=1):
                argmax_j, _ = max(enumerate(sentence.source_tokens, start=1),
                                key=lambda pair: self.translation_probs[pair[1]][tgt])
                alignment.append((argmax_j, i))
...
class WordPositionAligner(WordAligner):
    ...
    def _e_step(self, parallel_corpus):
        ...
        for sentence in parallel_corpus:
            ...
            phis = self._get_probs_for_lengths(src_len, trg_len) 
            theta = self.translation_probs[np.ix_(sentence.source_tokens, sentence.target_tokens)]
            res.append((phis * theta) / np.sum(phis * theta, axis = 0))
        return res
    ...
    def _compute_elbo(self, parallel_corpus, posteriors):
        ...
        for i in range(len(posteriors)):
            ...
            src_len = len(src_idx)
            tgt_len = len(tgt_idx)
            phis = self._get_probs_for_lengths(src_len, tgt_len)
            
            res += np.sum(posteriors[i] * (np.log(phis + 1e-8) + np.log(self.translation_probs[np.ix_(src_idx, tgt_idx)] + 1e-8)\
                                            - np.log(posteriors[i] + 1e-8)))
        return res
    ...
    def _m_step(self, parallel_corpus, posteriors):
        ...
        for i in range(len(parallel_corpus)):
            ...
            if (src_len, tgt_len) not in self.alignment_probs:
                self.alignment_probs[(src_len, tgt_len)] = posteriors[i]
            else:
                self.alignment_probs[(src_len, tgt_len)] += posteriors[i]
